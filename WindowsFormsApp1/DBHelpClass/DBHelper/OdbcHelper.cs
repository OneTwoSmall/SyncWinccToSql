...
namespace DBHelpClass.Helper
{
    ...
    public abstract class OdbcHelper
    {
        ...
        /// <summary>
        ///执行一个不需要返回值的OdbcCommand命令，通过指定专用的连接字符串。
        /// 使用参数数组形式提供参数列表 
        /// </summary>
        /// <param name="connectionString">一个有效的数据库连接字符串</param>
        /// <param name="cmdType">OdbcCommand命令类型 (存储过程， T-SQL语句， 等等。)</param>
        /// <param name="cmdText">存储过程的名字或者 T-SQL 语句</param>
        /// <param name="commandParameters">以数组形式提供OdbcCommand命令中用到的参数列表</param>
        /// <returns>返回一个数值表示此OdbcCommand命令执行后影响的行数</returns>
            ...
            params SqlParameter[] commandParameters)
        {
            ...
            {
                //通过PrePareCommand方法将参数逐个加入到OdbcCommand的参数集合中
        ...
        /// <summary>
        ///执行一个不需要返回值的OdbcCommand命令，通过指定专用的连接字符串。带上事务处理
        /// 使用参数数组形式提供参数列表 
        /// </summary>
        /// <param name="connectionString">一个有效的数据库连接字符串</param>
        /// <param name="cmdType">OdbcCommand命令类型 (存储过程， T-SQL语句， 等等。)</param>
        ...
        /// <summary>
        /// 执行一条返回结果集的OdbcCommand，通过一个已经存在的数据库连接
        /// 使用参数数组提供参数
        /// </summary>
        /// <param name="connecttionString">一个现有的数据库连接</param>
        /// <param name="cmdTye">OdbcCommand命令类型</param>
        /// <param name="cmdText">存储过程的名字或者 T-SQL 语句</param>
        /// <param name="commandParameters">以数组形式提供OdbcCommand命令中用到的参数列表</param>
            ...
            SqlParameter[] commandParameters)
        {
            ...
            using (OdbcConnection conn = new OdbcConnection(connecttionString))
            {
        ...
        {
            return GetTable(OdbcHelper.connectionString, cmdTye, cmdText, commandParameters);
        }
        ...
        /// 为执行命令准备参数
        /// </summary>
        /// <param name="cmd">OdbcCommand 命令</param>
        /// <param name="conn">已经存在的数据库连接</param>
        /// <param name="trans">数据库事物处理</param>
        /// <param name="cmdType">OdbcCommand命令类型 (存储过程， T-SQL语句， 等等。)</param>
        /// <param name="cmdText">Command text，T-SQL语句 例如 Select * from Products</param>
        /// <param name="cmdParms">返回带参数的命令</param>
        private static void PrepareCommand(OdbcCommand cmd, OdbcConnection conn, OdbcTransaction trans, CommandType cmdType,
        ...
        {
            return ExecuteScalar(OdbcHelper.connectionString, cmdType, cmdText, commandParameters);
        }
        ...
        {
            return ExecuteScalar(OdbcHelper.connectionString, CommandType.StoredProcedure, cmdText, commandParameters);
        }
        ...
        {
            return ExecuteScalar(OdbcHelper.connectionString, CommandType.Text, cmdText, commandParameters);
        }
        ...
        public static void SqlBulkCopyByDatatable(string TableName, DataTable dt)
        {
            ...
            using (OdbcBulkCopy sqlbulkcopy = new OdbcBulkCopy(connectionString, SqlBulkCopyOptions.UseInternalTransaction))
            {
                try
                {
        ...
        public static void SqlBulkUpdateByDatatable(string connectionStr, string TableName, DataTable dt, string primaryKey)
        {
            //string primaryKey = "ValueID";//主键可以作为参数传进来

            using (OdbcConnection conn = new OdbcConnection(connectionStr))
            {
                var tempTableName = $"##{TableName}";//全局临时表名
        ...
        public static void SqlBulkUpdateByDatatable(string TableName, DataTable dt, string primaryKey)
        {
            SqlBulkUpdateByDatatable(connectionString, TableName, dt, primaryKey);
        }
